...
import { useTheme, Palette, fonts, levelColors, levelBg } from "../theme";
...
function LogRow({ entry, colors }: { entry: LogEntry; colors: Palette }) {
  const styles = createStyles(colors);
  return (
    <View style={styles.logRow}>
      <Text style={styles.logTime}>{entry.time}</Text>
      <Text style={[styles.logLevel, { color: levelColors[entry.level], backgroundColor: levelBg[entry.level] }]}>
        {entry.level}
      </Text>
        ...
        {LEVELS.map((level) => {
          ...
          return (
            <AnimatedPressable
              ...
              scaleTo={0.94}
            >
              <Text style={[styles.levelChipLabel, { color: isActive ? colors.white : colors.ink }]}>{level}</Text>
            </AnimatedPressable>
          );
        })}
      </ChipRow>
...
function createStyles(colors: Palette) {
  return StyleSheet.create({
    levelChip: { paddingHorizontal: 12, paddingVertical: 5, borderWidth: 2, borderColor: colors.border },
    ...
    consoleHeader: { borderBottomWidth: 2, borderBottomColor: colors.border, backgroundColor: colors.ink, paddingHorizontal: 12, paddingVertical: 6 },
